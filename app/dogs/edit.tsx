...
import { StyleSheet, View, TextInput, Text, TouchableOpacity, ScrollView, Alert, Platform, Modal, FlatList, ActivityIndicator } from 'react-native';
...
export default function EditDogScreen() {
  ...
  useEffect(() => {
    async function fetchDog() {
      ...
      try {
        ...
        if (data) {
          setName(data.name || '');
          if (data.breed && !COMMON_BREEDS.includes(data.breed)) {
            setBreed('Autre');
            setOtherBreed(data.breed);
          } else {
            setBreed(data.breed || '');
          }
  ...
  async function updateDog() {
    ...
    if (error) {
      ...
      if(Platform.OS !== 'web') Alert.alert('Succès', 'Les informations ont été modifiées !');
      router.back();
    }
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]} contentContainerStyle={styles.contentContainer} keyboardShouldPersistTaps="handled">
      <View style={styles.header}>
         <TouchableOpacity onPress={() => router.back()} style={styles.backButton}>
           <Text style={[{ color: theme.tint, fontSize: 16 }]}>Annuler</Text>
         </TouchableOpacity>
         <Text style={[styles.title, { color: theme.text }]}>Modifier</Text>
      </View>
      ...
      <View style={styles.photoContainer}>
        <TouchableOpacity onPress={pickImage} style={[styles.photoButton, { borderColor: theme.icon }]}>
          {photoData ? (
             ...
             <Image source={{ uri: existingPhotoUrl }} style={styles.photoPreview} />
          ) : (
             <View style={styles.photoPlaceholder}>
                <IconSymbol name="camera.fill" size={30} color={theme.icon} />
                <Text style={{color: theme.icon, marginTop: 5, fontSize: 12}}>Changer la photo</Text>
             </View>
          )}
        </TouchableOpacity>
      </View>
      ...
      <View style={[styles.verticallySpaced, styles.mt20, { marginBottom: 40 }]}>
        <TouchableOpacity
